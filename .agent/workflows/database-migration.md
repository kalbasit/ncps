...
1. Determine the database type (sqlite, postgres, or mysql).

1. Run the creation command (example for sqlite). Do **NOT** use the `--url` flag for migration creation:
...
1. To run the migration, you MUST use the `--url` flag to specify the database connection:
