...
func getChunkStorageBackend(ctx context.Context, cmd *cli.Command) (chunk.Store, error) {
	localDataPath := cmd.String("cache-storage-local")
	s3Bucket := cmd.String("cache-storage-s3-bucket")
	...
	case s3Bucket != "":
		...
		return chunk.NewS3Store(ctx, s3Cfg)
	default:
...
func createCache(
	...
	if cdcEnabled {
		chunkStore, err := getChunkStorageBackend(ctx, cmd)
		if err != nil {
