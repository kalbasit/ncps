...
const createPinnedClosure = `-- name: CreatePinnedClosure :execresult
...
//	INSERT INTO pinned_closures (hash)
//	VALUES (?)
func (q *Queries) CreatePinnedClosure(ctx context.Context, hash string) (sql.Result, error) {
	return q.db.ExecContext(ctx, createPinnedClosure, hash)
}
...
type GetCDCNarFilesWithSizeMismatchRow struct {
	ID                int64
	Hash              string
	Compression       string
	FileSize          uint64
...
//	    AND ni.nar_size IS NOT NULL
//	    AND nf.file_size != CAST(ni.nar_size AS UNSIGNED)
func (q *Queries) GetCDCNarFilesWithSizeMismatch(ctx context.Context) ([]GetCDCNarFilesWithSizeMismatchRow, error) {
	rows, err := q.db.QueryContext(ctx, getCDCNarFilesWithSizeMismatch)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []GetCDCNarFilesWithSizeMismatchRow
	for rows.Next() {
		var i GetCDCNarFilesWithSizeMismatchRow
		if err := rows.Scan(
			&i.ID,
			&i.Hash,
			&i.Compression,
