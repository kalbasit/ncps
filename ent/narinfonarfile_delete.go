// Code generated by ent, DO NOT EDIT.

package ent

import (
	"context"

	"entgo.io/ent/dialect/sql"
	"entgo.io/ent/dialect/sql/sqlgraph"
	"entgo.io/ent/schema/field"
	"github.com/kalbasit/ncps/ent/narinfonarfile"
	"github.com/kalbasit/ncps/ent/predicate"
)

// NarInfoNarFileDelete is the builder for deleting a NarInfoNarFile entity.
type NarInfoNarFileDelete struct {
	config
	hooks    []Hook
	mutation *NarInfoNarFileMutation
}

// Where appends a list predicates to the NarInfoNarFileDelete builder.
func (_d *NarInfoNarFileDelete) Where(ps ...predicate.NarInfoNarFile) *NarInfoNarFileDelete {
	_d.mutation.Where(ps...)
	return _d
}

// Exec executes the deletion query and returns how many vertices were deleted.
func (_d *NarInfoNarFileDelete) Exec(ctx context.Context) (int, error) {
	return withHooks(ctx, _d.sqlExec, _d.mutation, _d.hooks)
}

// ExecX is like Exec, but panics if an error occurs.
func (_d *NarInfoNarFileDelete) ExecX(ctx context.Context) int {
	n, err := _d.Exec(ctx)
	if err != nil {
		panic(err)
	}
	return n
}

func (_d *NarInfoNarFileDelete) sqlExec(ctx context.Context) (int, error) {
	_spec := sqlgraph.NewDeleteSpec(narinfonarfile.Table, sqlgraph.NewFieldSpec(narinfonarfile.FieldID, field.TypeInt))
	if ps := _d.mutation.predicates; len(ps) > 0 {
		_spec.Predicate = func(selector *sql.Selector) {
			for i := range ps {
				ps[i](selector)
			}
		}
	}
	affected, err := sqlgraph.DeleteNodes(ctx, _d.driver, _spec)
	if err != nil && sqlgraph.IsConstraintError(err) {
		err = &ConstraintError{msg: err.Error(), wrap: err}
	}
	_d.mutation.done = true
	return affected, err
}

// NarInfoNarFileDeleteOne is the builder for deleting a single NarInfoNarFile entity.
type NarInfoNarFileDeleteOne struct {
	_d *NarInfoNarFileDelete
}

// Where appends a list predicates to the NarInfoNarFileDelete builder.
func (_d *NarInfoNarFileDeleteOne) Where(ps ...predicate.NarInfoNarFile) *NarInfoNarFileDeleteOne {
	_d._d.mutation.Where(ps...)
	return _d
}

// Exec executes the deletion query.
func (_d *NarInfoNarFileDeleteOne) Exec(ctx context.Context) error {
	n, err := _d._d.Exec(ctx)
	switch {
	case err != nil:
		return err
	case n == 0:
		return &NotFoundError{narinfonarfile.Label}
	default:
		return nil
	}
}

// ExecX is like Exec, but panics if an error occurs.
func (_d *NarInfoNarFileDeleteOne) ExecX(ctx context.Context) {
	if err := _d.Exec(ctx); err != nil {
		panic(err)
	}
}
