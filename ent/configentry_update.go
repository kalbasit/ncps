...
func (_u *ConfigEntryUpdate) check() error {
...
func (_u *ConfigEntryUpdate) sqlSave(ctx context.Context) (_node int, err error) {
	...
	if _u.mutation.UpdatedAtCleared() {
		_spec.ClearField(configentry.FieldUpdatedAt, field.TypeTime)
	}
...
// SetValue sets the "value" field.
func (_u *ConfigEntryUpdateOne) SetValue(v string) *ConfigEntryUpdateOne {
	_u.mutation.SetValue(v)
	return _u
}
...
// check runs all checks and user-defined validators on the builder.
func (_u *ConfigEntryUpdateOne) check() error {
	if v, ok := _u.mutation.Key(); ok {
		if err := configentry.KeyValidator(v); err != nil {
			return &ValidationError{Name: "key", err: fmt.Errorf(`ent: validator failed for field "ConfigEntry.key": %w`, err)}
		}
