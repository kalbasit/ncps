...
- **Goal**: a single de-chunk pass always reaches `chunked = 0`; de-chunked NARs are self-consistent; fsck can heal any leftover residue. No operator SQL.
...
- **(D) fsck repair pass.** Add a chunked-NAR residue check to fsck that, per chunked `nar_file`, relinks+normalizes when a valid NarHash exists or purges otherwise — the same heal logic, runnable independently of a drain.
