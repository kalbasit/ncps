...
#### Scenario: Cross-pod compressed request mid-chunking serves from staging, not 404

- **WHEN** replica A holds the download lock and is actively chunking an eager-CDC NAR for hash `H`
- **AND** replica B receives a request for a compressed variant (e.g. `.nar.xz`) of `H`
- **THEN** replica B SHALL contend for the lock, record an in-flight staging request, and serve the complete NAR from staging once parts are available
- **AND** replica B SHALL NOT return HTTP 404 by short-circuiting to chunk-based serving

#### Scenario: Uncompressed cross-pod request mid-chunking still streams progressively

- **WHEN** replica A is actively chunking an eager-CDC NAR for hash `H` and replica B receives a request for the uncompressed (`none`) variant
- **THEN** replica B SHALL serve progressively from chunks as they are committed, unchanged by the held-lock behavior
