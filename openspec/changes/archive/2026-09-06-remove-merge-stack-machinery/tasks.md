...
- [x] 1.3 Confirm the blast radius is exactly two files: `grep -rIn --exclude-dir=.git -e 'merge-stack' -e 'stackmerge' .` matches only `.github/workflows/merge-stack-start.yml` and `.github/workflows/merge-stack-continue.yml`.

## 2. Removal

- [x] 2.1 Delete `.github/workflows/merge-stack-start.yml`.
- [x] 2.2 Delete `.github/workflows/merge-stack-continue.yml`.
- [x] 2.3 Re-run the grep from 1.3 and confirm zero matches remain.
...
- [ ] 4.4 On the pull request, confirm the `ci` required check reports and is green, and that no `Merge Stack` workflow is dispatched.

## 5. Close out

- [x] 5.1 Archive this change (`/opsx:archive`) so `openspec-guard` passes and the pull request can merge.
- [ ] 5.2 Report the `monitor-stack-merge` follow-up to the operator: `~/.claude/skills/monitor-stack-merge/SKILL.md` still instructs adding the `merge-stack` label and polling `merge-stack-continue.yml`, and must be rewritten against native stacks. Out of scope here — it lives outside the repository.
