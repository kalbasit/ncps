...
### D7 — GC / retention
Staging parts are reclaimed once the final representation (whole NAR or chunks, per mode) is committed **plus** `inflight-staging-retention` grace, draining in-flight tailers first. Trigger is event-driven on ingest completion, backed by a periodic sweep keyed off `staging_state.updated_at` staleness (falling back to `staging_state.created_at` when never updated) + `status` to catch holder deaths — a live holder advances `parts_available`, and thus `updated_at`, as it stages, so staleness (not age) is the liveness signal (parts whose holder died and was never taken over).
