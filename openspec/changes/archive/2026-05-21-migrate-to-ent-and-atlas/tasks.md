...
The statically-enforced invariants in this change are A1, A2, and A4 — the three for which `ent/schema/` contains triggering patterns today. A3, A5, snake-case enum-type naming, expand-contract DDL, and CHECK-presence cross-validation are documented project conventions (see `CLAUDE.md` and the `ent-schema-lint` spec) tracked by code review. They are intentionally **out of scope** for this change because the current schema tree has no triggering pattern for any of them (no edge-bound unique fields, no `*_ciphertext` columns, no `field.Enum(...)` declarations, and migrations are reviewed manually). The lint checks for these invariants will land in a follow-up change at the point a triggering pattern is introduced.

- [x] 5.1 Create `cmd/ent-lint/testdata/` with positive and negative fixture directories for each of A1, A2, A4
- [x] 5.2 Write `cmd/ent-lint/main_test.go` asserting one `[FAIL]` line per negative fixture and all `[PASS]` lines for the positive fixtures (covers A1, A2, A4)
...
- [x] 7.4 Generate `atlas.sum` for each dialect directory via the Atlas library call — done in §6 via `cmd/generate-migrations`'s `ensureAtlasSum` helper; all three `migrations/<dialect>/atlas.sum` files are committed and verified by the `atlas-sum-check` derivation (§13.3)
...
- [x] 8.6 `TestSchemaCreateMatchesAppliedMigrations` (Option E's fresh-install ↔ applied-migrations equivalence) — subsumed by the combination of §8.5 (`TestSchemaEquivalence` proves applied-migrations ≡ Ent's `migrate.Tables`) and §9.4 (`TestMigrateUp/<dialect>/fresh_install` proves `Schema.Create` + version seeding lands the same `migrate.Tables` and leaves goose with no pending versions). The two together transitively guarantee fresh-install ≡ applied-migrations across all three dialects.
