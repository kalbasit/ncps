# helm-ha-staging-validation

## Purpose

Define how the Helm chart's high-availability validation guard (`replicaCount > 1`) is satisfied. High availability requires a mechanism to serve a NAR across replicas while it is still downloading; this is provided by either CDC or in-flight NAR staging. The legacy `iLoveTimeouts` bypass is removed.
