...
ncps generates per-dialect SQL migrations from Ent schemas using Atlas (consumed as a Go library) and applies them at runtime using Goose. This capability covers the on-disk layout of migration files, the generation workflow, the runtime apply path (including in-place adoption of pre-existing dbmate-format tracking tables), the bridge migration that transitions dbmate-era schemas to the Ent-expected schema, the fresh-install `Schema.Create` path, and the expand-contract policy governing safe DDL changes.
