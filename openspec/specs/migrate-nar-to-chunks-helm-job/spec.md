# migrate-nar-to-chunks-helm-job Specification

## Purpose

Defines the Helm chart Job resource for the `migrate-nar-to-chunks` operation — an opt-in Kubernetes Job that runs `ncps migrate-nar-to-chunks` as a regular release resource (not a Helm hook), enabling operators to perform forward CDC migrations within their existing GitOps/ArgoCD workflows.
