# container-image-etc-files

## Purpose

Guarantees the ncps OCI image is startable on container runtimes that strictly validate symlinks during container creation, by shipping the create-time-critical `/etc` identity files as regular files rather than nix-store symlinks.
