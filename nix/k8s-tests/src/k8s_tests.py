...
class K8sTestsCLI:
    def __init__(self, verbose: bool = False):
        ...
        self.config_file = os.environ.get("CONFIG_FILE", os.path.join(REPO_ROOT, "nix/k8s-tests/config.nix"))
    ...
    def cmd_cluster_create(self):
        ...
        # Create Kind Cluster
        clusters = self.run_cmd(["kind", "get", "clusters"], capture_output=True).stdout.splitlines()
...
- role: control-plane
  ...
  - containerPort: 30000
    ...
    protocol: TCP
"""
            self.run_cmd(["kind", "create", "cluster", "--name", CLUSTER_NAME, "--config", "-"], input=kind_config)
        ...
        self.run_cmd(["helm", "upgrade", "--install", "minio", "minio/minio",
        ...
        """
        self.run_cmd(["kubectl", "run", "minio-configurator", "--namespace", "minio", "--image=minio/mc",
                      "--restart=Never", "--rm", "-i", "--command", "--", "/bin/sh", "-c", mc_cmd])
...
"""
        ...
        self.run_cmd(["helm", "upgrade", "--install", "cnpg", "cnpg/cloudnative-pg", "--namespace", "cnpg-system", "--create-namespace", "--wait"])
        self.run_cmd(["helm", "upgrade", "--install", "mariadb-operator-crds", "mariadb-operator/mariadb-operator-crds", "--namespace", "mariadb-system", "--create-namespace", "--wait"])
        self.run_cmd(["helm", "upgrade", "--install", "mariadb-operator", "mariadb-operator/mariadb-operator", "--namespace", "mariadb-system", "--create-namespace", "--set", "webhook.cert.certManager.enabled=false", "--wait"])
        self.run_cmd(["helm", "upgrade", "--install", "redis-operator", "ot-helm/redis-operator", "--namespace", "redis-system", "--create-namespace", "--wait"])
...
"""
    ...
    def _wait_for_pods(self, label: str, ns: str):
        ...
        for _ in range(30):
            res = self.run_cmd(["kubectl", "get", "pod", "-l", label, "-n", ns, "-o", "name"], capture_output=True, check=False)
            if res.stdout.strip():
                break
            time.sleep(2)
        else:
            self.error(f"Timed out waiting for pods with label '{label}' to appear in namespace '{ns}'")

        # Wait for condition with retries for transient failures (e.g., pod deleted while waiting)
        for _ in range(5):
            try:
                self.run_cmd(["kubectl", "wait", "--for=condition=Ready", "pod", "-l", label, "-n", ns, "--timeout=180s"])
                return
            except subprocess.CalledProcessError:
                self.log(f"   ⚠️  'kubectl wait' failed, retrying...")
                time.sleep(5)

        self.error(f"Timed out waiting for pods with label '{label}' to be ready in namespace '{ns}'")
    ...
    def cmd_cluster_info(self, json_output: bool = False):
        ...
        self.log("\n--- 🪣 S3 (MinIO) ---")
        for k, v in creds["s3"].items(): self.log(f"  {k.capitalize()}: {v}")
        self.log("\n--- 🐘 PostgreSQL 17 ---")
        for k, v in creds["postgresql"].items(): self.log(f"  {k.capitalize()}: {v}")
        self.log("\n--- 🐬 MariaDB ---")
        for k, v in creds["mariadb"].items(): self.log(f"  {k.capitalize()}: {v}")
        self.log("\n--- 🔺 Redis ---")
    ...
    def cmd_generate(self, push: bool, last: bool, tag: Optional[str], registry: str, repository: str):
        image_tag = tag
        ...
        if push:
            ...
            build_path = self.run_cmd(["nix", "build", f"{REPO_ROOT}#docker", "--print-out-paths", "--no-link"], capture_output=True).stdout.strip()
            ...
            self.run_cmd(["skopeo", "--insecure-policy", "copy", "--dest-tls-verify=false", f"docker-daemon:{nix_image}", f"docker://{full_image}"])
        ...
        nix_eval_cmd = [
            "nix", "eval", "--json", "--file", self.config_file, "generateValues",
            "--apply", f"f: f (builtins.fromJSON ''{args_json}'')"
        ]
...
"""
    ...
    def _generate_test_config(self, creds, permutations):
        ...
        test_config = {
            "cluster": creds,
            "test_data": {
                "narinfo_hashes": test_data_hashes
            },
            "deployments": []
        }
    ...
    def cmd_install(self, name: Optional[str]):
        ...
        for n in sorted(names):
            ...
            self.run_cmd(["helm", "upgrade", "--install", f"ncps-{n}", CHART_DIR,
                          "-f", values_file, "--create-namespace", "--namespace", f"ncps-{n}"])
    ...
    def cmd_cleanup(self, name: Optional[str]):
        if name:
            ...
            self.run_cmd(["helm", "uninstall", f"ncps-{name}", "-n", f"ncps-{name}"], check=False)
            self.run_cmd(["kubectl", "delete", "namespace", f"ncps-{name}"], check=False)
        else:
            self.log("🧹 Cleaning up all test deployments...")
            ns_list = self.run_cmd(["kubectl", "get", "namespaces", "-o", "jsonpath={.items[*].metadata.name}"], capture_output=True).stdout.split()
            for ns in ns_list:
    ...
    def cmd_test(self, name: Optional[str]):
        ...
        from k8s_tests_tester import NCPSTester # We'll extract this to a helper or include it
    ...
    def cmd_all(self):
        ...
        self.cmd_generate(push=True, last=False, tag=None, registry="127.0.0.1:30000", repository="ncps")
...
def main():
    ...
    cluster_sub.add_parser("create").set_defaults(func=lambda cli, _: cli.cmd_cluster_create())
    cluster_sub.add_parser("destroy").set_defaults(func=lambda cli, _: cli.cmd_cluster_destroy())
    ...
    gen_p.set_defaults(func=lambda cli, args: cli.cmd_generate(args.push, args.last, args.tag, args.registry, args.repository))
