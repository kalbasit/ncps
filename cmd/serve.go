...
var (
	...
	ErrStorageConfigRequired = errors.New("either --cache-data-path or --storage-s3-bucket is required")

	// ErrS3ConfigIncomplete is returned if S3 bucket is set but required S3 config is missing.
	ErrS3ConfigIncomplete = errors.New("S3 requires --storage-s3-endpoint, access-key-id, and secret-access-key")

	// ErrStorageConflict is returned if both local and S3 storage are configured.
	ErrStorageConflict = errors.New("cannot use both --cache-data-path and --storage-s3-bucket")
)
...
func serveCommand(userDirs userDirectories, flagSources flagSourcesFn) *cli.Command {
	return &cli.Command{
		...
		Flags: []cli.Flag{
			...
			&cli.BoolFlag{
				Name:    "storage-s3-use-ssl",
				Usage:   "Use SSL/TLS for S3 connection (default: true)",
				Sources: flagSources("storage.s3.use-ssl", "STORAGE_S3_USE_SSL"),
				Value:   true,
			},
			&cli.StringFlag{
...
func createCache(
	...
	case s3Bucket != "":
		// Use S3 storage
		s3Endpoint := cmd.String("storage-s3-endpoint")
		s3AccessKeyID := cmd.String("storage-s3-access-key-id")
		s3SecretAccessKey := cmd.String("storage-s3-secret-access-key")

		if s3Endpoint == "" || s3AccessKeyID == "" || s3SecretAccessKey == "" {
			return nil, ErrS3ConfigIncomplete
		}

		// Parse endpoint to determine SSL usage
		useSSL := cmd.Bool("storage-s3-use-ssl")
		endpoint := s3.GetEndpointWithoutScheme(s3Endpoint)

		// If the endpoint has a scheme, use that to determine SSL
		if s3.IsHTTPS(s3Endpoint) {
			useSSL = true
		} else if len(s3Endpoint) > 7 && s3Endpoint[:7] == "http://" {
			useSSL = false
		}
